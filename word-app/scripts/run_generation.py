...
def git_commit():
    try:
        subprocess.run(['git', 'add', 'public/images/'], capture_output=True)
        result = subprocess.run(['git', 'add', 'public/images/*.jpg'], capture_output=True)
        total = len([f for f in os.listdir(IMAGE_DIR) if f.endswith('.jpg')])
        commit_msg = f"feat: {total} images generated"
        subprocess.run(['git', 'commit', '-a', '-m', commit_msg], capture_output=True)
        print(f"\n📦 Git committed: {total} images total")
    except Exception as e:
        pass

def batch_generate(batch_size=50, max_workers=8):
...
if __name__ == "__main__":
    ...
    print("🚀 开始持续图片生成...按 Ctrl+C 停止")
    while True:
        has_more = batch_generate(batch_size=batch_size, max_workers=max_workers)
        if not has_more:
            break
        print("\n😴 等待 3秒...\n")
        time.sleep(3)
