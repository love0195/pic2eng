        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=30) as response:
                ...
                if size == PLACEHOLDER_SIZE:
                    print(f"  尝试 {attempt}/24: 占位图 ({size} bytes), 等待...")
                    time.sleep(6)
                elif size > 10000 and content[:2] == b'\xff\xd8':
                    with open(filepath, 'wb') as f:
                        f.write(content)
                    print(f"  ✅ 成功: {word} ({size} bytes)")
                    return True
                else:
                    print(f"  尝试 {attempt}/24: 大小异常 ({size} bytes), 等待...")
                    time.sleep(4)
                        
        except Exception as e:
            print(f"  尝试 {attempt}/24: 错误={e}, 重试...")
            time.sleep(4)
...
print("\n" + "=" * 70)
print("⏳ 等待 120 秒让AI生成图片（增加等待时间）...")
print("=" * 70)
time.sleep(120)
print("✅ 等待结束！\n")

print("=" * 70)
print("第二步：下载所有真实图片（最多等待5分钟）")
print("=" * 70)
