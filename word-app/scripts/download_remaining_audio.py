...
def main():
    ...
    print(f"📝 复合词会分别下载每个部分的音频\n")
    
    # 加载缺失列表
    missing_file = BASE_DIR / '.final_missing_audio.json'
    ...
    words_to_download = sorted(list(all_words))
    
    print(f"📋 原始缺失列表: {len(missing_words)} 个")
    print(f"📋 扩展后（含复合词拆分）: {len(words_to_download)} 个单词\n")
    
    # 分成批次处理，每30个单词提交一次
    batch_size = 30
    total_batches = (len(words_to_download) + batch_size - 1) // batch_size
    
    print(f"📊 总共 {len(words_to_download)} 个单词，分为 {total_batches} 个批次\n")
    ...
    for batch_num in range(total_batches):
        ...
        # 提交这一批
        if batch_success > 0:
            git_commit(f"下载音频 - 第 {batch_num + 1}/{total_batches} 批 (成功: {total_success}/{len(words_to_download)})")
        else:
